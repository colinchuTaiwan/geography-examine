...
def save_record(name: str, score: int, difficulty: str) -> str:
    ...
    firebase_db.reference("records_geography").push({
        "id":         record_id,
        "name":       name,
        "score":      score,
...
def load_records_cached() -> list:
    init_firebase()
    data = firebase_db.reference("records_geography").get()
    if not data:
        return []
...
st.set_page_config(page_title="地理測驗挑戰網", page_icon="🌏", layout="centered")
